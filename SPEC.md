...
V30: remote-state — azurerm backend `use_azuread_auth`; bootstrap via `az` CLI not terraform ! `infra/backend/`; RG `rg-credit-policy-tfstate` container `tfstate`; account `sttfstlab5` ! workload `stcp*`; reconstructible after clone; blob key `<env>.tfstate`; first workload `terraform init -migrate-state` local → remote; `*.tfstate` gitignored never committed
...
T15|.|Makefile infra-create emit `infra/outputs.json`; talos+pytest fill missing env from file; drop live `terraform output`; gitignore file; infra-destroy drop file|V15,I.infra,I.env,I.cmd
