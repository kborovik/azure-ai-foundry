...
- names: env ∈ {dev1, prd1}; RG `rg-credit-policy-${env}`; resourceToken literal `lab5` ! md5; storage `stcplab5`; search `srch-cp-lab5` SKU basic; Foundry `aif-cp-lab5` kind AIServices S0; project `credit-policy-demo`; chat deploy `gpt-5-mini` sku GlobalStandard capacity 50 (=50k TPM); embed `text-embedding-3-large` capacity 20; KS `ks-credit-policies` + `ks-client-applications`; KB `kb-credit-policies`; conn `conn-kb-credit-policies`; agent `credit-policy-agent`; blob containers `credit-policies` + `client-applications`
...
T22|.|swap terraform `local.resource_token` md5(subscription-env-location) → literal `lab5`; `gmake infra-destroy` then `gmake infra-create`|I.names,I.infra,V11
