...
V32: application-generate — `talos generate application` calls Foundry `gpt-5-mini`; unique SyntheticBorrower (name, fictional address, `SYN-######` customer_id, `@example.invalid` email); `--type` or `--all` hidden operator constraint; `--force` overwrite slot; facts.yaml literals drive `accepted`|`rejected`|`missing-data`; Markdown = customer filing (identity, product, amount, financials, attached-docs titles only — ! generate attached files); `accepted`/`rejected` list full required set; `missing-data` omit ≥1 required title; mixed product family per slot; intended_outcome in gitignored `manifest.json` only; ! ApplicationType or judgement in MD body or YAML; file `credit-application-{application_id}.md`; validate+1 retry; ! real PII
