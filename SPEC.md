...
V15: env-contract — Bicep outputs + pytest + talos read only canonical env names; flags override process env; missing keys from `azd env get-values` unless `--no-azd`; never overwrite set keys; ! load repo-root `.env`
...
T11|.|drop `.env` load from talos+pytest; flags>process>azd; drop `.env.example`; keep gitignore `.env`|V15,V17,I.env,I.cmd,I.pytest
