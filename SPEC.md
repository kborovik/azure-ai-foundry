...
- pytest: markers `unit` `ingestion` `retrieval` `agent` `teams`; load repo-root `.env` for missing keys; live skip w/o canonical env; teams skip unless `E2E_TEAMS=1` (flag set and no client → skip not fail)
...
V15: env-contract — Bicep outputs + pytest + talos read only canonical env names; flags override process env; missing keys from repo-root `.env` if present (strip quotes, skip blank/#) then `azd env get-values` unless `--no-azd`; never overwrite set keys
...
V17: secrets — Microsoft Entra ID / DefaultAzureCredential operator path; keys may exist on resource; never commit keys/connection strings; repo-root `.env` gitignored; ! `disableLocalAuth: true`
