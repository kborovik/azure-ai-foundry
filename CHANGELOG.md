...
- **Make recipes:** Rename `gmake infra` to `gmake infra-create` and `gmake infra-backend` to `gmake infra-backend-create`. Add `gmake infra-show` and `gmake infra-backend-show` (`terraform show`).
...
- **`gmake infra-backend-create` / `infra-backend-show` / `infra-backend-destroy` / `infra-destroy`:** bootstrap the tfstate account (prefix `sttfst`, never workload `stcp*`); show or destroy the workload stack or the backend. First workload `terraform init` uses `-migrate-state` when a local `infra/terraform.tfstate` exists.
